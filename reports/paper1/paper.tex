...
The running times of the filtering are shown in Fig.~\ref{fig:inference_timing}. The running times are compared with the mean-variance filter which normalises the statistics using the sample mean and sample standard deviation. Compared to the mean-variance filter, the empirical null filter was slow and much slower for larger kernel radiuses.
...
Voids in the order of millimetres were detectable which shows potential in defect detection in projection space. This
work is a contribution towards real-time quality control in AM.
