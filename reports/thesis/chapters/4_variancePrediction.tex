...
Assuming the model is correct, it is given that $D\sim\chi_{N-k}^2$ which implies that for large $N$,
\begin{equation}
\expectation\left[D_\mathrm{s}\right] = \dfrac{1}{\alpha}
\end{equation}
and
\begin{equation}
\variance\left[D_\mathrm{s}\right] = \dfrac{2}{\alpha^2 N} \ .
\end{equation}
...
\begin{figure}
  ...
  \caption{Absolute value residuals from fitting a gamma GLM of the form ${y}(x)=\beta_0+\beta_1 x$ onto the sample variance-mean data obtained from the replicate projections. Highlighted in a red box is an example of an inflection. The colour scales are in units of \SI{}{\adu^2}.}
