...
The sample variance-mean data were obtained from the replicate projections. Let $x_{i,j}^{(r)}$ be the grey value of the pixel at $(i,j)$ from the $r$th replicate projection for $r=1,2,3,\dotsc,n$. The sample mean and sample variance grey value for the pixel at $(i,j)$ are
\begin{equation}
...
\begin{equation}
  ...
  \ .
\end{equation}
where $\widehat{y}_i=\widehat{y}(x_i)$. $\alpha=(n-1)/2$ was assumed to be known so does not need to be estimated.

The procedure is as follows. A criterion and a link function was chosen beforehand. In the initial step, a GLM with systematic component $\eta(x)=\beta_0$ was fitted and the criterion was recorded. In the next step, a polynomial feature with one order higher was added to the systematic component $\eta(x)=\beta_0+\beta_1 x$, fitted and the criterion recorded. In addition, a polynomial feature with one order lower was added $\beta(x)=\beta_{-1}x^{-1}+\beta_0$ and fitted separately with the criterion recorded. The model which decreased the criterion the most was accepted. Adding higher and lower order polynomials to the systematic component was repeated, for example after accepting $\eta(x)=\beta_0+\beta_1 x$, the following systematic components $\eta(x)=\beta_0+\beta_1 x+\beta_2x^2$ and $\eta(x)=\beta_{-1}x^{-1}+\beta_0+\beta_1 x$ were fitted and assessed. The fitted parameters, $\beta_0$ and $\beta_1$ in this example, may change when adding more terms. This is repeated until the criterion cannot be decreased and the procedure is left with the final model.
...
Instead of assessing the model fit using the AIC and BIC, the performance on predicting the variance given a grey value was assessed using cross validation \citep{friedman2001elements}. Cross validation assesses the model to make predictions on data it has not seen before. This was done by randomly splitting the $m$ data points into two disjoint sets, the training set and the test set. It was chosen that the training and test set are of the same size. The model was fitted onto the training set. Afterwards, the fitted model predicts the variances given the grey values in the test set which were then compared to the actual variances. 
...
Assuming the model is correct, it is given that $D\sim\chi_{m-k}^2$ which implies that for large $m$,
\begin{equation}
...
\begin{figure}
  ...
  \caption{Test mean scaled deviance from predicting variances in a test set using a GLM fitted onto a training set. The box plot represent the 100 repeats of the analysis by bootstrapping the replicate projections.}
...
A residual analysis was conducted to study for anything overlooked. The model ${y}(x)=\beta_0+\beta_1 x$ was fitted onto the entire sample variance-mean data. A residual plot plots $r_i=y_i-\widehat{y}(x_i)$ for all $x_i$ in the dataset which was done using a 2D histogram. Due to the gamma GLM, higher grey values typically have bigger residuals in magnitude. A predicted residual interval is included in the histogram to aid judging the residuals. This interval was acquired by obtaining the prediction interval and subtracting it from the fit.
...
\begin{figure}
  ...
  \caption{Log frequency density histogram of the residuals given the mean grey value. The residuals are from fitting a gamma GLM of the form ${y}(x)=\beta_0+\beta_1 x$ onto the sample variance-mean data obtained from the replicate projections. The dashed red line shows the 68\% confidence interval of the predicted residual.}
  \label{fig:meanVar_residualGreyvalue}
\end{figure}

The residual plots are shown in Figure \ref{fig:meanVar_residualGreyvalue}. The residuals are all sensible and increased in magnitude with grey value which is captured by the predicted residual interval. The inflection in \texttt{AbsNoFilter} at \ang{120}, as discussed previously, can be seen more clearly here.
...
\begin{figure}
  ...
  \caption{Absolute value residuals from fitting a gamma GLM of the form ${y}(x)=\beta_0+\beta_1 x$ onto the sample variance-mean data obtained from the replicate projections. Highlighted in a red box is an example of an inflection.}
