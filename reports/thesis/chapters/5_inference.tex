...
In this section, the single hypothesis test will be extended and the multiple hypothesis testing \citep{shaffer1995multiple} shall be reviewed.
...
The Bonferroni correction \citep{shaffer1995multiple, bland1995multiple, perneger1998what} controls the family wise error rate. This is done by adjusting the size of the test to be $\alpha/n$. By using the adjusted size, Equation \eqref{eq:inference_fwer} becomes
\begin{equation}
...
\end{equation}
This shows that the Bonferroni correction controls the family-wise error rate. However in practice the Bonferroni correction is not very powerful \citep{perneger1998what}, meaning it does give too many false negative results. This is because the correction traded too many false positives for false negatives.

The \cite{benjamini1995controlling} (BH) procedure controls the false discovery rate (FDR) \citep{benjamini2010discovering} rather than the false positive or family-wise rate. The FDR is the proportion of false positives out of all positive results.
...
\begin{equation}
    ...
    \ .
\end{equation}
This essentially declare $p_{(1)},p_{(2)},\dotdotdot,p_{(k)}$ as positive results. For completeness, if $p_{(1)}>\alpha/n$, then $k=1$ and the Bonferroni correction is recovered. The procedure comes from the fact that assuming the null hypothesis is true for all $Z_i$, then the $p$-values are uniformly distributed \citep{simes1986improved}.

It can be shown that the BH procedure controls the FDR such that
\begin{equation}
    \expectation[\text{FDR}]\leqslant\pi_0\alpha\leqslant\alpha
\end{equation}
where $\pi_0$ is the proportion of true nulls out of $n$ tests \citep{benjamini1995controlling}, this is usually unknown in practice.
