Disagreement between the obtained and the \emph{aRTist} simulated projections can be found by subtracting one projection from the other, any values too big in magnitude can be considered as a defect. However in the previous chapters, it was found that x-ray photons behave randomly and differences in the comparison can be due to chance. Thus the comparisons should be done under the face of uncertainty.
...
Model misspecification appeared to be the main source of error. The test statistics and $p$-values were inspected in Figure \ref{fig:inference_inferenceIntro_histogram}. It can be seen that the majority of the test statistics does come from a Normal distribution. Also, the majority of the $p$-values are not look uniformly distributed. This seems to suggest that the assumption of $Z_{x,y}\sim \normal(0,1)$ is incorrect.
...
Consider a small example where a $200\times200$ section of the $z$ image was investigated, as shown Figure \ref{fig:inference_inferenceSubsample1_histogram}. The distribution of the test statistics appeared Normal but not centred at zero. So by using the BH procedure to obtain a critical region of $|Z|>\input{../figures/inference/inferenceSubsample1_criticalBoundary.txt}$, almost half of the pixels were tested positive. It is questionable whether such a number of positive results is sensible, in particular in an area where defects were not expected.
...
The null mean was estimated by numerically finding the mode of the density estimate. The null standard deviation was estimated using Equation \eqref{eq:inference_nullStdEstimator}. This results in the empirical null density to have the same curvature as the density estimate at the mode, this is illustrated in Figure \ref{fig:inference_inferenceSubsample1_densityEstimate}. In this particular example, it was found that $\widehat{\mu}_0=\input{../figures/inference/inferenceSubsample1_nullMean.txt}$ and $\widehat{\sigma}_0=\input{../figures/inference/inferenceSubsample1_nullStd.txt}$ to 2 decimal places.
...
\begin{figure}
  \centering
    ...
    \caption{Histogram of the test statistics from the sub-image in Figure \ref{fig:inference_inferenceSubsample1_histogram}. Also shown are the critical regions and $p$-values which were adjusted using the empirical null.}
    \label{fig:inference_inferenceSubsample1_nullResults}
\end{figure}

No positive results were found and the $p$-values were sensible as they resemble the uniformly distribution. This demonstrated that the empirical null adjusted the parameters of the null distribution to fit onto the majority of the data in order to make sensible inference.
...
Another example is shown in Figure \ref{fig:inference_inferenceSubsample2} where a $200\times 200$ sub-image containing a defect was used. It can be seen that the null distribution was not centred at zero, however the empirical null took it into account. The estimation of the empirical null parameters were robust as it depends on the density estimate at the mode only, it should not be affected by non-null test statistics.
...
\begin{figure}
  \centering
    ...
    \caption{A smoothing spline was fitted on the log squared error for estimating $\sigma_0$ on $n$ simulated standard Normal random variables. The bandwidth was varied. The experiment was repeated by simulating the standard Normal random variables again. The boxplots represent the variation of the error from the 100 repeats. The red dashed line shows the bandwidth which minimised the fitted spline.}
...
\begin{figure}[p]
  ...
  \caption{Different types of central tendency estimators were used on $\pi r^2$ standard Normal random variables. Shown are the sampling distributions from 100 repeats. The dotted lines show the 68\% confidence interval for the sample mean.}
...
\begin{figure}
  ...
  \caption{Different types of statistical dispersion estimators were used on $\pi r^2$ standard Normal random variables. Shown are the sampling distributions from 100 repeats. The dotted lines show the 68\% confidence interval for the sample variance.}
...
\begin{figure}[p]
  ...
  \caption{Different types of statistical dispersion estimators were used on $\pi r^2$ mixture of Gaussian random variables. The random variable has distribution $\normal(0,1)$ with probability $0.9$ and $\normal(3,1)$ with probability $0.1$. Shown are the sampling distributions from 100 repeats. The dotted lines show the 68\% confidence interval for the sample variance.}
...
\begin{figure}
  \centering
    ...
    \caption{Various dispersion filters with kernel radius $r=5$ on the sample image \texttt{bridge.gif}.}
    \label{fig:inference_fijiStd}
\end{figure}

When using the the empirical null filter in \emph{ImageJ}, the user is presented with a menu, as shown in Figure \ref{fig:inference_fijiGui}. Here the user can input the kernel radius, the filter then modifies the currently selected image $z_{x,y}$ to $t_{x,y}$ for all $x$ and $y$ using a circular kernel with the specified radius. The menu also has options for by-product images to be shown such as: the empirical null mean $\widehat{\mu}_{0,x,y}$, the empirical null standard deviation $\widehat{\sigma}_{0,x,y}$, the standard deviation filter, and quartile filters. Advanced options are available to the user. The number of initial values set the number of valid solutions to be found in the Newton-Raphson method on finding the mode of the density estimate. The user can also set the maximum number of steps and tolerance for the Newton-Raphson method. The bandwidth parameters for the kernel density estimate can also be adjusted here.
...
\begin{figure}
  ...
  \caption{The within image sample \emph{mean} of the test statistics in a filtered Gaussian image of size $256\times256$. The box plots summarise the 100 repeats of the experiment. The dashed lines show the 68\% confidence interval using standard tests, assuming independence.}
...
\begin{figure}
  ...
  \caption{The within image sample \emph{standard deviation} of the test statistics in a filtered Gaussian image of size $256\times256$. The box plots summarise the 100 repeats of the experiment. The dashed lines show the 68\% confidence interval using standard tests, assuming independence.}
...
\begin{figure}
  ...
  \caption{The within image sample \emph{kurtosis} of the test statistics in a filtered Gaussian image of size $256\times256$. The box plots summarise the 100 repeats of the experiment. The dashed lines show the 68\% confidence interval using standard tests, assuming independence.}
...
\begin{figure}
  ...
  \caption{The within image sample \emph{mean} of the test statistics in a filtered \emph{contaminated} Gaussian image of size $256\times256$. The box plots summarise the 100 repeats of the experiment. The dashed lines show the 68\% confidence interval using standard tests, assuming independence.}
...
\begin{figure}
  ...
  \caption{The within image sample \emph{standard deviation} of the test statistics in a filtered \emph{contaminated} Gaussian image of size $256\times256$. The box plots summarise the 100 repeats of the experiment. The dashed lines show the 68\% confidence interval using standard tests, assuming independence.}
...
\begin{figure}
  ...
  \caption{The within image sample \emph{kurtosis} of the test statistics in a filtered \emph{contaminated} Gaussian image of size $256\times256$. The box plots summarise the 100 repeats of the experiment. The dashed lines show the 68\% confidence interval using standard tests, assuming independence.}
...
In terms of AUC, the empirical null filter improved the performance over using the unfiltered contaminated image. The performance before contamination cannot be recovered but it was an improvement. For kernel radiuses too small, such as Figure \ref{fig:inference_defectSimulationRoc_square20}, the empirical null filter deteriorated the performance and it would be better off using the contaminated image.

\subsection{Speckle Defect Experiment}

The ROC curves considers all thresholds or specificities used in the hypothesis testing. In the previous experiment, it was found there was some variance mismatch before and after filtering with regards to the test statistics. As a result, for a given threshold such as the 5\% FDR level, the specificity may change ever so slightly after filtering.

An experiment was conducted to investigate the specificity of the test before and after filtering. A $256\times 256$ Gaussian image with speckle defect with density $\pi_1=0.1$ and various values of $\mu_1$ were investigated. The AUC, type 1 error, type 2 error and FDR were measured when testing on an image with a defect before and after contamination and then after filtering with the contamination. A kernel radius of $r=20$ was used and was repeated 100 times by simulating another image.
...
\begin{figure}[p]
  ...
  \caption{AUC and various errors obtained when conducting hypothesis test on the filtered contaminated image of a $256 \times 256$ line defected Gaussian image using various kernel radiuses. The dashed lines show the resulting 68\% empirical confidence interval when the hypothesis test was done on the uncontaminated image. Defected pixels have the distribution $\normal(3,1)$. The line defect is 5 pixels thick. The contamination is such that the null distribution is $Z_{x,y}|H_{0,x,y}\sim\normal(\mu_{0,x,y},\sigma_0^2)$ where $\mu_{0,x,y} = 0.01 (x-x_0) + 0.01 (y-y_0)$ and $\sigma_0=2$. The test was done at the 5\% FDR level. The box plot summarise the 100 repeats of the experiment.}
...
\begin{figure}[p]
  \centering
    \centerline{
      \begin{subfigure}[b]{\subSize}
        \includegraphics[width=1.05\textwidth, height=5.45cm]{../figures/inference/DefectDetectAbsFilterDeg120_radius1_sig.eps}
        \caption{Image with positive pixels}
      \end{subfigure}
      \begin{subfigure}[b]{\subSize}
        \includegraphics[width=\textwidth]{../figures/inference/DefectDetectAbsFilterDeg120_radius1_logp.eps}\vspace{-1mm}
        \caption{$-\log p$}
      \end{subfigure}
    }
    \centerline{
  ...
  \caption{Resulting inference when using the empirical null filter on the \texttt{AbsFilter} at \ang{120} projection. A kernel radius of $r=\addNumber{../figures/inference/DefectDetectAbsFilterDeg120_radius1.txt}$ was used. a) Highlighted in red are positive pixels at the 5\% FDR level. b) $p$-values on the log scale. c) and d) Empirical null mean and deviation.}
...
\begin{figure}[p]
  \centering
    \centerline{
      \begin{subfigure}[b]{\subSize}
        \includegraphics[width=1.05\textwidth, height=5.45cm]{../figures/inference/DefectDetectAbsFilterDeg120_radius4_sig.eps}
        \caption{Image with positive pixels}
      \end{subfigure}
      \begin{subfigure}[b]{\subSize}
        \includegraphics[width=\textwidth]{../figures/inference/DefectDetectAbsFilterDeg120_radius4_logp.eps}\vspace{-1mm}
        \caption{$-\log p$}
      \end{subfigure}
    }
    \centerline{
  ...
  \caption{Resulting inference when using the empirical null filter on the \texttt{AbsFilter} at \ang{120} projection. A kernel radius of $r=\addNumber{../figures/inference/DefectDetectAbsFilterDeg120_radius4.txt}$ was used. a) Highlighted in red are positive pixels at the 5\% FDR level. b) $p$-values on the log scale. c) and d) Empirical null mean and deviation.}
  \label{fig:inference_AbsFilterDeg120_inference_radius4}
\end{figure}

The empirical null filter was applied to the \texttt{AbsFilter} projection at \ang{120}. Figures \ref{fig:inference_AbsFilterDeg120_inference_radius1} and \ref{fig:inference_AbsFilterDeg120_inference_radius4} shows the resulting inference when using a kernel radius of $r=\addNumber{../figures/inference/DefectDetectAbsFilterDeg120_radius1.txt}$ and $r=\addNumber{../figures/inference/DefectDetectAbsFilterDeg120_radius4.txt}$ respectively.
With a small radius, the empirical null treated the voids as the null which is shown by the empirical null mean capturing the features of the voids. When a large enough radius was used, the larger voids were highlighted by the hypothesis test and the empirical null mean became smoother. This is evident that this method ironed out the false positives observed at the start of the chapter.
...
False positives were detected on the bottom right of the test sample. This was the result of the null distribution being bimodal where each face had a different distribution. Figure \ref{fig:inference_cornerSelect} illustrates this. The geometry of the faces and the kernel used was such that it resulted in one of the faces to be treated as non-null and then tested positive, falsely so.

\begin{figure}[p]
  \centering
    \centerline{
      \begin{subfigure}[b]{\subSize}
        \includegraphics[width=1.05\textwidth, height=5.45cm]{../figures/inference/DefectDetectSubRoiAbsFilterDeg120_radius4_sig.eps}
        \caption{Image with positive pixels}
      \end{subfigure}
      \begin{subfigure}[b]{\subSize}
  ...
  \caption{Resulting inference when using the empirical null filter on the \texttt{AbsFilter} at \ang{120} projection and on each segment independently. A kernel radius of $r=\addNumber{../figures/inference/DefectDetectSubRoiAbsFilterDeg120_radius4.txt}$ was used. a) Highlighted in red are positive pixels at the 5\% FDR level. The test statistics from all segments were combined in the BH procedure. b) $p$-values on the log scale. c) and d) Empirical null mean and standard deviation.}
...
\begin{figure}[p]
  \centering
    \centerline{
      \begin{subfigure}[b]{\subSize}
        \includegraphics[width=1.05\textwidth, height=5.45cm]{../figures/inference/DefectDetectSubRoiAbsFilterDeg30_radius1_sig.eps}
        \caption{Image with positive pixels}
      \end{subfigure}
      \begin{subfigure}[b]{\subSize}
        \includegraphics[width=\textwidth]{../figures/inference/DefectDetectSubRoiAbsFilterDeg30_radius1_logp.eps}\vspace{-1mm}
        \caption{$-\log p$}
      \end{subfigure}
    }
    \centerline{
...
\subsection{Titanium Test Sample}

The whole procedure was repeated on the titanium test sample in the dataset \texttt{TiFilter}. The resulting $z$ statistics are shown in Figure \ref{fig:inference_subroiTiFilter_unfiteredZ} and shows, as in the \texttt{AbsFilter} case, a boundary on the edges of the test sample. The faces of the test sample were manually segmented and the empirical null filter was used on each segment separately and stitched together to form the final stages of the analysis.

\begin{figure}
  \centering
    \centerline{
      \begin{subfigure}[b]{\subSize}
        \includegraphics[width=\textwidth]{../figures/inference/DefectDetectSubRoiTiFilterDeg30_unfilteredZ.eps}
        \caption{\ang{30}}
      \end{subfigure}
      \begin{subfigure}[b]{\subSize}
        \includegraphics[width=\textwidth]{../figures/inference/DefectDetectSubRoiTiFilterDeg120_unfilteredZ.eps}
        \caption{\ang{120}}
      \end{subfigure}
    }
  \caption{Unfiltered $z$ images from the dataset \texttt{TiFilter}}
  \label{fig:inference_subroiTiFilter_unfiteredZ}
\end{figure}
...
\begin{figure}[p]
  \centering
    \centerline{
      \begin{subfigure}[b]{\subSize}
        \includegraphics[width=1.03\textwidth, height=5.45cm]{../figures/inference/DefectDetectSubRoiTiFilterDeg30_radius1_sig.eps}
        \caption{Image with positive pixels}
      \end{subfigure}
      \begin{subfigure}[b]{\subSize}
        \includegraphics[width=\textwidth]{../figures/inference/DefectDetectSubRoiTiFilterDeg30_radius1_logp.eps}\vspace{-1mm}
        \caption{$-\log p$}
      \end{subfigure}
    }
    \centerline{
      \begin{subfigure}{\subSize}
        \includegraphics[width=\textwidth]{../figures/inference/DefectDetectSubRoiTiFilterDeg30_radius1_nullMean.eps}
        \caption{Empirical null mean}
      \end{subfigure}
      \begin{subfigure}{\subSize}
        \includegraphics[width=\textwidth]{../figures/inference/DefectDetectSubRoiTiFilterDeg30_radius1_nullStd.eps}
        \caption{Empirical null std}
      \end{subfigure}
    }
  \caption{Resulting inference when using the empirical null filter on the \texttt{TiFilter} at \ang{30} projection and on each segment independently. A kernel radius of $r=\addNumber{../figures/inference/DefectDetectSubRoiTiFilterDeg30_radius1.txt}$ was used. a) Highlighted in red are positive pixels at the 5\% FDR level. The test statistics from all segments were combined in the BH procedure. b) $p$-values on the log scale. c) and d) Empirical null mean and standard deviation.}
  \label{fig:inference_subroiTiFilterDeg30_inference}
\end{figure}
...
\begin{figure}[p]
  \centering
    \centerline{
      \begin{subfigure}[b]{\subSize}
        \includegraphics[width=1.03\textwidth, height=5.45cm]{../figures/inference/DefectDetectSubRoiTiFilterDeg120_radius1_sig.eps}
        \caption{Image with positive pixels}
      \end{subfigure}
      \begin{subfigure}[b]{\subSize}
        \includegraphics[width=\textwidth]{../figures/inference/DefectDetectSubRoiTiFilterDeg120_radius1_logp.eps}\vspace{-1mm}
        \caption{$-\log p$}
      \end{subfigure}
    }
    \centerline{
      \begin{subfigure}{\subSize}
        \includegraphics[width=\textwidth]{../figures/inference/DefectDetectSubRoiTiFilterDeg120_radius1_nullMean.eps}
        \caption{Empirical null mean}
      \end{subfigure}
      \begin{subfigure}{\subSize}
        \includegraphics[width=\textwidth]{../figures/inference/DefectDetectSubRoiTiFilterDeg120_radius1_nullStd.eps}
        \caption{Empirical null std}
      \end{subfigure}
    }
  \caption{Resulting inference when using the empirical null filter on the \texttt{TiFilter} at \ang{120} projection and on each segment independently. A kernel radius of $r=\addNumber{../figures/inference/DefectDetectSubRoiTiFilterDeg120_radius1.txt}$ was used. a) Highlighted in red are positive pixels at the 5\% FDR level. The test statistics from all segments were combined in the BH procedure. b) $p$-values on the log scale. c) and d) Empirical null mean and standard deviation.}
  \label{fig:inference_subroiTiFilterDeg120_inference}
\end{figure}

The resulting inference are shown in Figures \ref{fig:inference_subroiTiFilterDeg30_inference} and \ref{fig:inference_subroiTiFilterDeg120_inference}. Unfortunately the edges of the test sample were falsely tested as positive and none of the voids were detected. The discrepancies are the edges can be due to inaccurate face segmentation or poor alignment of the \emph{aRTist} projection to the obtained projection.
