...
Disagreement between the scan and \texttt{aRTist} can be found by subtracting one image from the other, any values too big in magnitude can be considered as a defect. However in the previous chapters, it was found that x-ray photons behave randomly and differences in the comparison can be due to chance. Thus the comparisons should be done under the face of uncertainty.

A pixel by pixel inference was proposed to do defect detection. A test statistic $Z_{x,y}$ for the $(x,y)$ positioned pixel was calculated for all pixels. This test statistic is
\begin{equation}
  Z_{x,y} = 
  \dfrac{
...
This method for inference was applied to the dataset \texttt{Sep16 120deg}. A cuboid (4 cm $\times$ 4 cm $\times$ 6 cm) with voids was purposefully manufactured to see if they can be detected, the CAD shown in Figure \ref{fig:inference_testObject}. The voids were of various diameters 2.4 mm, 1.2 mm, 0.6 mm and 0.3 mm. 6 voids for each diameter were designed in the CAD. Voids with diameters 2.4 mm and 0.6 mm were regularly arranged, otherwise they were arranged irregularly. The precision of the manufacturing was in the order of $\pm0.1\text{ mm}$.
...
\begin{figure}
  ...
  \caption{An x-ray image of a 3D printed cuboid, from the \texttt{Sep16 120deg} dataset, was compared to the \texttt{aRTist} simulation to detect the purposefully manufactured voids. The $p$-values obtained were used for hypothesis testing, using the \cite{benjamini1995controlling} procedure, at the 5\% FDR level. Pixels detected as positive are shown in red in d).}
  \label{fig:inference_inferenceIntro_inference}
\end{figure}

For each pixel, a test statistic was calculated. A test statistic too large in magnitude will classify that pixel as a positive result. In two-tailed hypothesis testing \citep{pearson1900on, neyman1933on, fisher1970statistical}, the $p$-value can be used to represent test statistic in a different way
\begin{equation}
...
\begin{figure}[htp]
  ...
  \caption{a) The histogram of the test statistics is compared with the standard Normal distribution. The standard Normal distribution is known as the null distribution in this scenario. b) The $p$-values are ordered and plotted. The critical region corresponds to controlling the false discovery rate at 5\% using the \cite{benjamini1995controlling} procedure. The dotted line shows the result if the $p$-values were uniformly distributed.}
  \label{fig:inference_inferenceIntro_histogram}
\end{figure}

This chapter recaps hypothesis testing, for a single test and then for multiple tests, treating each pixel as a test. Assumptions used on the hypothesis testing can be relaxed by using the empirical null \citep{efron2004large}, which is reviewed here. The empirical null is then extended to an image filter, called the empirical null filter. This filter adjust each test statistic according to its neighbours, ironing out false positive results. Simulations and results are shown towards the end of the chapter.

\section{Literature Review}

Consider a test statistic from a pixel. If there are no defects, then the statistic is null and has the null distribution $Z|H_0\sim\normal(0,1)$. This can be described in another way by specifying the random variable as $Z\sim\normal(\mu,1)$ and the null hypothesis as $H_0:\mu=0$. A hypothesis test can judge how much $Z$ deviates from 0 by defining the alternative hypothesis $H_1:\mu\neq 0$. A statistic which does not have the null distribution is known as non-null.
...
Multiple hypothesis testing occurs when there are more than one hypotheses to test. For example $n$ pixels can be tested where the test statistics are $Z_1, Z_2, \cdots, Z_n$ and $Z_i\sim\normal(\mu_i,1)$ for $i=1,2,\cdots,n$. The null hypotheses are $H_{0,i}:\mu_i=0$ and are tested against the alternative $H_{1,i}:\mu_i\neq 0$ for $i=1,2,\cdots,n$. Let the corresponding $p$-values be $p_1, p_2, \cdots, p_n$.
...
Consider a small example where a $200\times200$ section of the $z$ image was investigated, as shown Figure \ref{fig:inference_inferenceSubsample1_histogram}. The distribution of the test statistics looked Normal but not centred at zero, as a result using the BH procedure obtained a critical region of $|Z|>\input{../figures/inference/inferenceSubsample1_criticalBoundary.txt}$. Almost half of the pixels were tested positive. It is questionable whether such a number of positive results is sensible, in particular in an area where defects were not expected.
...
\begin{figure}[htp]
  \centering
    ...
    \caption{The resulting $z$ image is shown in a). A $200\times 200$ sub-image was taken shown by the dashed lines. A histogram of the test statistics in the sub-image is shown in b). The critical region was obtained by using the BH procedure at the 5\% FDR level.}
...
This method does not need estimations of $\pi_0$ which makes it quite convenient. The estimations of $\pi_0$ is discussed in literature such as \cite{benjamini2000adaptive, pounds2003estimating, storey2003statistical, pounds2004improving, langaas2005estimating, durnez2014posthoc}. \cite{efron2004large} motives the use of the empirical null by investigating various types of false discovery rates \citep{storey2002direct, storey2003positive, efron2002empirical, efron2007size} which will not be discussed here.

The empirical null is used widely, including in neuroimaging \citep{schwartzman2008false, schwartzman2009empirical} and has been extended to include non-Normal null distributions \citep{schwartzman2008false, schwartzman2008empirical}. There exist various methods for the empicial null. \cite{schwartzman2008empirical} uses Poisson regression on the histogram counts in regions where the null statistics dominate. The empirical characteristic function was used in \cite{jin2007estimating}.
...
\end{equation}
where $s_z$ and $\text{IQR}_z$ are the sample standard deviation and sample interquartile range.
...
The test statistics were normalised to $T_1,T_2,\cdots,T_n$ by using
\begin{equation}
  T_i = \dfrac{
    Z_i - \widehat{\mu}_0
  }
...
\begin{figure}
  \centering
    ...
    \caption{The critical regions and $p$-values were adjusted using the empirical null in the sub-image example.}
...
\begin{figure}
  \centering
    ...
    \caption{A $200\times 200$ sub-image containing a defect is shown in a). The empirical null was used to test pixels, positive pixels are shown in red in b). The histogram of the test statistics are shown in c) along with the critical region. The normalised $p$-values are shown in d). The critical regions were adjusted using the empirical null and obtained using the BH procedure at the 5\% FDR level.}
...
The mode was found by solving $\widehat{\mu}_0 = \argmax\widehat{p}_Z(z)$. This cannot be solved in closed form easily and was done numerically. The  Newton-Raphson method can be used to solve $\hat{p}_Z'(z) = 0$ for $z$. Instead the log space was used, solving
\begin{equation}
  \dfrac{
    \partial
  }
...
\end{equation}
for $r=0,1,2,3,\cdots$ until some convergence condition is met. The derivatives for the log density was obtained via the following. Recall that the kernel density estimate is $\widehat{p}_Z(z)=
  \frac{1}{nh}
  \sum_{i=1}^n\phi\left(
    \frac{z_i-z}{h}
  \right) $
...
\begin{figure}
  \centering
    ...
    \caption{A smoothing spline was fitted on the log squared error for estimating $\sigma_0$ against the bandwidth. Boxplots represent the variation of the error from 100 repeats. The red dashed line shows the bandwidth which minimised the fitted spline.}
...
The GLM fit in Figure \ref{fig:inference_BandwidthSelection2_ruleOfThumb} appeared to underfit, which suggest there may be a more complicated relationship between the optimal bandwidth and $n$. The method used in fitting the smoothing spline could contribute to the underfitting. For example if the smoothing spline was not smooth enough so that local minima appeared, it would add variance to the location of the optimal bandwidth. A basic linear relationship was chosen to avoid overfitting and keep the rule of thumb simple.
...
To test for robustness, a dataset of $n = \pi r^2$ mixture of Gaussian random variables were simulated with the following distribution: $\normal(0,1)$ with probability $90\%$, $\normal(3,1)$ with probability $10\%$. The $\normal(3,1)$ component acts as non-null statistics and robust estimators should be unaffected by it. The results are shown in Figures \ref{fig:inference_NullIidMixtureMean} and \ref{fig:inference_NullIidMixtureStd}. The empirical null estimators are robust to the non-null component but still suffers from high variance. The rest of the estimators were affected by the non-null statistics, making them unsuitable to estimate parameters of the null component.
...
\begin{figure}[p]
  ...
  \caption{Different types of central tendency estimators were used on $\pi r^2$ standard Normal random variables. Shown are the sampling distributions from 100 repeats. The dotted lines show the 95\% confidence interval for the sample mean.}
...
\begin{figure}
  ...
  \caption{Different types of statistical dispersion estimators were used on $\pi r^2$ standard Normal random variables. Shown are the sampling distributions from 100 repeats. The dotted lines show the 95\% confidence interval for the sample variance.}
...
Returning back to the example at the start of the chapter, the empirical null could be used on the histogram shown in Figure \ref{fig:inference_inferenceIntro_histogram}. The resulting empirical null distribution is shown in Figure \ref{fig:inference_inferenceSubsampleAll}. The empirical null distribution had a very large standard deviation and would test all pixels as negative, this is incorrect.
...
The problem was that the empirical distribution of the test statistics did not look Normal. It appeared that the test statistics varied spatially, suggesting the empirical null varied spatially as well. One possible reason is because it looked like the value of the test statistics depended on which surface it was on for example.

One possible extension to the empirical null is to spilt the $z$ image into a grid and conduct inference in each section separately. There are problems with this though. The first problem is that how the grid is overlaid can be arbitrary, for example the grid can be translated to produce different sections. Secondly it is not clear how to combine the results from each section together \citep{efron2008simultaneous}.
...
\end{equation}
for $x=1,2,\cdots,W$ and $y=1,2,\cdots,H$. $\mu_{0,x,y}$ and $\sigma_{0,x,y}$ are the null mean and null standard deviation at position $(x,y)$ respectively. Define the null hypotheses to be
\begin{equation}
...
The empirical null filter uses the empirical null on neighbouring pixels to estimate the null parameters. To estimate $\mu_{0,x,y}$ and $\sigma_{0,x,y}$, a circular kernel of radius $r$ was centred at $(x,y)$. All the pixels captured by the circular kernel were used for the empirical null to obtain $\widehat{\mu}_{0,x,y}$ and $\widehat{\sigma}_{0,x,y}$, in other words
\begin{align}
  \widehat{\mu}_{0,x,y}
  &=
  \widehat{\mu}_{0,x,y}
...
\\
  ...
  \ .
\end{align}
Using these estimates, the test statistics were normalised using
\begin{equation}
  T_{x,y} = 
  \dfrac{
...
There exist similar filters, for example the local normalization filter \citep{sage2003teaching, sage2018local} uses a Gaussian filter to obtain $\widehat{\mu}_{0,x,y}$. $\widehat{\sigma}_{0,x,y}^2$ was obtained by applying another Gaussian filter on the $\left(z_{x,y}-\widehat{\mu}_{0,x,y}\right)^2$ image. The purpose of this filter was to normalise images with uneven illumination. The disadvantage of using the local normalization filter for estimating the null parameters is that it uses a Gaussian filter. This is a type weighted mean which won't be robust to non-null statistics. In addition, the two Gaussian filters can have different radiuses, it is not clear how to set them.

\subsection{Design and Implementation}

The empirical null filter was implemented in an open source software called \texttt{ImageJ} \citep{abramoff2004image, schneider2012nih, perez2013image} by modifying the existing class \texttt{RankFilters}, available on \texttt{GitHub} \citep{imagej2018rank}. This class implemented filters, such as the mean filter and median filter, using a circular kernel, making this a suitable framework for the empirical null filter. Figure \ref{fig:inference_fijiGui} shows the graphical user interface of the empirical null filter in \texttt{Fiji} \citep{schindelin2012fiji}, a distribution of \texttt{ImageJ}.
...
As a test, the empirical null filter with $r=5$ was used on a \texttt{ImageJ} sample image \texttt{bridge.gif}, as shown in Figure \ref{fig:inference_fijiBridgeFilter}. It is meaningless to use the empirical null filter on an image not for the purpose of the filter, however it did verify that the filter worked on arbitrary images with some computational considerations. The empirical null images $\widehat{\mu}_{0,x,y}$ and $\widehat{\sigma}_{0,x,y}$ for all ${x,y}$ could be of interest. In particular the empirical null mean image could be interpreted as the result of a mode filter \citep{griffin2000mean}. Figure \ref{fig:inference_fijiMean} compares the empirical null mean with other averaging filters. The empirical null mean created an impasto effect and preserved edges which is similar to the result in \cite{griffin2000mean}. Spread filters such as the standard deviation filter can be used to detect edges. Figure \ref{fig:inference_fijiStd} compares the standard deviation filter with the empirical null standard deviation. The resulting images were similar but it was notable that the edges in the empirical null standard deviation were sharper, suggesting that the empirical null standard deviation is a more robust measure of spread compared to the standard deviation.
...
When using the the empirical null filter in \texttt{ImageJ}, the user is presented with a menu, as shown in Figure \ref{fig:inference_fijiGui}. Here the user can input the kernel radius, the filter then modifies the currently selected image $z_{x,y}$ to $t_{x,y}$ for all $x$ and $y$ using a circular kernel with the specified radius. The menu also has options for by-product images to be shown such as: the empirical null mean $\widehat{\mu}_{0,x,y}$, the empirical null standard deviation $\widehat{\sigma}_{0,x,y}$, the standard deviation filter, and quartile filters. Advanced options are available to the user. The number of initial values set the number of valid solutions to be found in the Newton-Raphson method on finding the mode of the density estimate. The user can also set the maximum number of steps and tolerance for the Newton-Raphson method. The bandwidth parameters for the kernel density estimate can also be adjusted here.
...
\begin{figure}[p]
  ...
  \caption{A $256\times 256$ Gaussian image before and after filtering with kernel radius 20. Also shown are the empirical null mean and empirical null standard deviation.}
...
\begin{figure}[p]
  ...
  \caption{A $256\times 256$ contaminated Gaussian image before and after filtering with kernel radius 20. Also shown are the empirical null mean and empirical null standard deviation.}
...
\begin{figure}[htp]
  ...
  \end{subfigure}
  \caption{$p$-values from a filtered $256\times 256$ Gaussian image with kernel radius 20. In b), the Gaussian image was contaminated before filtering. The $p$-values were obtained from the images in Figures \ref{fig:inference_allNullGaussianScript_afterFilter} and \ref{fig:inference_allNullPlaneScript_afterFilter}. The dotted lines shows the $p$-values if they were uniformly distributed.}
  \label{fig:inference_allNull_pValues}
\end{figure}

An example of a Gaussian image with/without contamination before/after filtering are shown in Figures \ref{fig:inference_allNullGaussianScript} and \ref{fig:inference_allNullPlaneScript}. In the contaminated example, the filter managed to estimate the null mean, picking up the gradient. This enabled the normalised image to look flat to remove the contamination. The $p$-values after filtering are shown in Figure \ref{fig:inference_allNull_pValues}. A quick inspection suggest that the filtered pixels looked reasonably Normal for this particular example.

For a given kernel radius $r$, 100 different Gaussian images were filtered to investigate the within image mean, standard deviation and kurtosis of the filtered statistics. Various other filters were used as well such as the MAD mode null filter, median IQR null filter and the mean variance null filter. They use different estimators to estimate the null parameters as their name suggest, Table \ref{table:inference_nullFilters} clarifies them.

\begin{table}[ht]
  \centering
  \begin{tabular}{c|c|c}
    Filters&Null mean&Null Standard deviation\\\hline
    MAD mode null&Mode&MAD around the mode $\times1.4826$\\
...
\afterpage{\clearpage}
\subsection{Detecting Defects Experiment}

In this experiment, the empirical null filter was tested to see if it can detect simulated defects from an image with/without contamination. A \emph{defect} is a pixel which has a value not distributed under the null distribution, but instead an alternative distribution. For example suppose $Z_{x,y}|H_{0,x,y}\sim\normal(0,1)$, then a defect would have a value distributed as $Z_{x,y}|H_{1,x,y}\sim\normal(\mu_{1},1)$ where $\mu_1\neq0$.

To recap, \emph{contamination} is the result of a linear transform of the test statistics $Z_{x,y}$. For example in this experiment, the image was multiplied by 2 and a gradient was added to it. Suppose the image does contain defects, then an example of a resulting distribution is
\begin{align}
  Z_{x,y}|H_{0,x,y}&\sim\normal(\mu_{0,x,y},2^2)
  \\
...
Various defects were investigated. Speckle defect with density $\pi_1$ assign all test statistics to be non-null with probability $\pi_1$, and are null otherwise. This was chosen because the proportion of null statistics captured by the kernel should be the same all for all kernel radiuses. A line defect assign columns of pixels to be non-null, a kernel would only capture a section of defect. Different shapes were used such as a square defect. With a square defect, a kernel can capture the entire defect if its radius is large enough.

\begin{figure}[p]
  \centering
    \centerline{
      \begin{subfigure}{0.49\textwidth}
        \includegraphics[width=\textwidth]{../figures/inference/defectDustExample_imagePreContaminated.eps}
        \caption{Uncontaminated}
      \end{subfigure}
      \begin{subfigure}{0.49\textwidth}
        \includegraphics[width=\textwidth]{../figures/inference/defectDustExample_imageContaminated.eps}
        \caption{Contaminated}
      \end{subfigure}
    }
    \centerline{
      \begin{subfigure}{0.49\textwidth}
        \includegraphics[width=\textwidth]{../figures/inference/defectDustExample_imageFiltered.eps}
        \caption{Filtered}
      \end{subfigure}
      \begin{subfigure}{0.49\textwidth}
        \includegraphics[width=\textwidth]{../figures/inference/defectDustExample_nullStd.eps}
        \caption{Empirical null std}
      \end{subfigure}
    }
  \caption{A $256 \times 256$ Gaussian image with \emph{speckle} defect is shown in a). The image was contaminated in b) and then filtered with kernel radius 20 in c). Defected pixels have the distribution $\normal(3,1)$. The speckle defect has density $\pi_1 = 0.1$. The contamination is such that the null distribution is $Z_{x,y}|H_{0,x,y}\sim\normal(\mu_{0,x,y},\sigma_0^2)$ where $\mu_{0,x,y} = 0.01 (x-x_0) + 0.01 (y-y_0)$ and $\sigma_0=2$. Highlighted in red are pixels tested as positive at the 5\% FDR level.}
...
\begin{figure}[p]
  \centering
    \centerline{
      \begin{subfigure}{0.49\textwidth}
        \includegraphics[width=\textwidth]{../figures/inference/defectLineExample_imagePreContaminated.eps}
        \caption{Uncontaminated}
      \end{subfigure}
      \begin{subfigure}{0.49\textwidth}
        \includegraphics[width=\textwidth]{../figures/inference/defectLineExample_imageContaminated.eps}
        \caption{Contaminated}
      \end{subfigure}
    }
    \centerline{
      \begin{subfigure}{0.49\textwidth}
        \includegraphics[width=\textwidth]{../figures/inference/defectLineExample_imageFiltered.eps}
        \caption{Filtered}
      \end{subfigure}
      \begin{subfigure}{0.49\textwidth}
        \includegraphics[width=\textwidth]{../figures/inference/defectLineExample_nullStd.eps}
        \caption{Empirical null std}
      \end{subfigure}
    }
  \caption{A $256 \times 256$ Gaussian image with a \emph{line} defect is shown in a). The image was contaminated in b) and then filtered with kernel radius 20 in c). Defected pixels have the distribution $\normal(3,1)$. The line defect is 5 pixels thick. The contamination is such that the null distribution is $Z_{x,y}|H_{0,x,y}\sim\normal(\mu_{0,x,y},\sigma_0^2)$ where $\mu_{0,x,y} = 0.01 (x-x_0) + 0.01 (y-y_0)$ and $\sigma_0=2$. Highlighted in red are pixels tested as positive at the 5\% FDR level.}
...
\begin{figure}[p]
  \centering
    \centerline{
      \begin{subfigure}{0.49\textwidth}
        \includegraphics[width=\textwidth]{../figures/inference/defectSquareExample_imagePreContaminated.eps}
        \caption{Uncontaminated}
      \end{subfigure}
      \begin{subfigure}{0.49\textwidth}
        \includegraphics[width=\textwidth]{../figures/inference/defectSquareExample_imageContaminated.eps}
        \caption{Contaminated}
      \end{subfigure}
    }
    \centerline{
      \begin{subfigure}{0.49\textwidth}
        \includegraphics[width=\textwidth]{../figures/inference/defectSquareExample_imageFiltered.eps}
        \caption{Filtered}
      \end{subfigure}
      \begin{subfigure}{0.49\textwidth}
        \includegraphics[width=\textwidth]{../figures/inference/defectSquareExample_nullMean.eps}
        \caption{Empirical null mean}
      \end{subfigure}
    }
  \caption{A $256 \times 256$ Gaussian image with a \emph{square} defect is shown in a). The image was contaminated in b) and then filtered with kernel radius \emph{20} in c). Defected pixels have the distribution $\normal(3,1)$. The square defect is $30\times 30$ pixels in size. The contamination is such that the null distribution is $Z_{x,y}|H_{0,x,y}\sim\normal(\mu_{0,x,y},\sigma_0^2)$ where $\mu_{0,x,y} = 0.01 (x-x_0) + 0.01 (y-y_0)$ and $\sigma_0=2$. Highlighted in red are pixels tested as positive at the 5\% FDR level.}
...
\begin{figure}[p]
  \centering
    \centerline{
      \begin{subfigure}{0.49\textwidth}
        \includegraphics[width=\textwidth]{../figures/inference/defectSquare2Example_imagePreContaminated.eps}
        \caption{Uncontaminated}
      \end{subfigure}
      \begin{subfigure}{0.49\textwidth}
        \includegraphics[width=\textwidth]{../figures/inference/defectSquare2Example_imageContaminated.eps}
        \caption{Contaminated}
      \end{subfigure}
    }
    \centerline{
      \begin{subfigure}{0.49\textwidth}
        \includegraphics[width=\textwidth]{../figures/inference/defectSquare2Example_imageFiltered.eps}
        \caption{Filtered}
      \end{subfigure}
      \begin{subfigure}{0.49\textwidth}
        \includegraphics[width=\textwidth]{../figures/inference/defectSquare2Example_nullMean.eps}
        \caption{Empirical null mean}
      \end{subfigure}
    }
  \caption{A $256 \times 256$ Gaussian image with a \emph{square} defect is shown in a). The image was contaminated in b) and then filtered with kernel radius \emph{40} in c). Defected pixels have the distribution $\normal(3,1)$. The square defect is $30\times 30$ pixels in size. The contamination is such that the null distribution is $Z_{x,y}|H_{0,x,y}\sim\normal(\mu_{0,x,y},\sigma_0^2)$ where $\mu_{0,x,y} = 0.01 (x-x_0) + 0.01 (y-y_0)$ and $\sigma_0=2$. Highlighted in red are pixels tested as positive at the 5\% FDR level.}
  \label{fig:inference_defectSquare2Example}
\end{figure}

An example of a speckle defected image is shown in Figure \ref{fig:inference_defectDustExample}, the figure also shows the image contaminated and then filtered. The contamination caused some systematic errors in hypothesis testing, such as an increase of positive pixels and in particular in two of the corners of the image. The empirical null filter recovered the image from the contaminated image so that appropriate inference was done. However some statistical power was lost in the filtered image. This is because in areas of high empirical null standard deviation, the normalised statistics became too small which then decreased the detection power. This cannot be avoided as areas of high empirical null standard deviation was due to the sampling error. Similar comments can be made for the line defect as shown in Figure \ref{fig:inference_defectLineExample}.

The square defected images, including the contaminated ones, are shown in Figures \ref{fig:inference_defectSquareExample} and \ref{fig:inference_defectSquare2Example}. Figure \ref{fig:inference_defectSquareExample} filtered the contaminated image using a kernel radius of $r=20$, while Figure \ref{fig:inference_defectSquare2Example} used a kernel radius of $r=40$. With $r=20$, the kernel is smaller than the square defect, therefore the defect was treated as the null and this evident in the empirical null mean. This resulted in difficulty detecting the defect. With $r=40$, the kernel is bigger than the defect and treated the defect as non-null. The empirical null filter recovered the gradient in the empirical null which was then used for normalisation.

The receiver operating characteristic (ROC) curves \citep{green1966signal, metz1978basic, hanley1982meaning, friedman2001elements, cook2007use} are shown in Figure \ref{fig:inference_defectSimulationRoc} for the various defects and filters. The ROC curve is a parametric plot, plotting the true positive rate (sensitivity) against the false positive rate ($1-\text{specificity}$) for varying thresholds. The area under the ROC curve is a commonly used statistic to quantify the performance of the test \citep{friedman2001elements}. Interpretations of the area do exist \citep{metz1978basic,hanley1982meaning} and discussed thoroughly in \cite{cook2007use}.

\begin{figure}[htp]
  \centering
    \centerline{
      \begin{subfigure}{0.49\textwidth}
        \includegraphics[width=\textwidth]{../figures/inference/defectDustExample_roc.eps}
        \caption{Dust defect $\pi_1=0.1$, $r=20$}
      \end{subfigure}
      \begin{subfigure}{0.49\textwidth}
        \includegraphics[width=\textwidth]{../figures/inference/defectLineExample_roc.eps}
        \caption{5 pixels thick line defect, $r=20$}
      \end{subfigure}
    }
    \centerline{
      \begin{subfigure}{0.49\textwidth}
        \includegraphics[width=\textwidth]{../figures/inference/defectSquareExample_roc.eps}
        \caption{$30\times 30$ square defect, $r=20$}
        \label{fig:inference_defectSimulationRoc_square20}
      \end{subfigure}
      \begin{subfigure}{0.49\textwidth}
        \includegraphics[width=\textwidth]{../figures/inference/defectSquare2Example_roc.eps}
        \caption{$30\times 30$ square defect, $r=40$}
      \end{subfigure}
    }
  \caption{ROC curves for various defected $256\times 256$ Gaussian images. The upper/lower dashed line show the resulting ROC curve when testing on an image without/with contamination respectively. The different curves are the resulting ROC curves after filtering a contaminated image. Defected pixels have the distribution $\normal(3,1)$. The contamination is such that the null distribution is $Z_{x,y}|H_{0,x,y}\sim\normal(\mu_{0,x,y},\sigma_0^2)$ where $\mu_{0,x,y} = 0.01 (x-x_0) + 0.01 (y-y_0)$ and $\sigma_0=2$.}
  \label{fig:inference_defectSimulationRoc}
\end{figure}

In terms of the area under the ROC curves, the empirical null filter improved the performance over using the unfiltered contaminated image. The performance before contamination cannot be recovered but it was an improvement. For kernel radiuses too small, such as Figure \ref{fig:inference_defectSimulationRoc_square20}, the empirical null filter deteriorated the performance and it would be better off using the contaminated image.

The ROC considers all thresholds or specificities used in the hypothesis testing. However in the previous experiment, it was found there was some variance mismatch before and after filtering with regards to the test statistics. As a result, for a given threshold such as the 5\% FDR level, the specificity may change ever so slightly after filtering.

An experiment was conducted to investigate the specificity of the test before and after filtering. A $256\times 256$ Gaussian image with speckle defect with density $\pi_1=0.1$ and various values of $\mu_1$ were investigated. Before the experiment, various baselines were obtained. An image with a defect was simulated and then tested to obtain the ROC area, type 1 error, type 2 error and the FDR. The image was than contaminated and tested to obtain the 4 measurements again. This was repeated 300 times to obtain the empirical distribution of the ROC area, type 1 error, type 2 error and FDR when testing on a un/contaminated image with defects. The ROC area, type 1 error, type 2 error and FDR were measured when testing on a filtered contaminated image with defects, a kernel radius of $r=20$ was used and was repeated 100 times.
...
\begin{figure}[p]
  ...
  \caption{ROC area and various errors obtained when conducting hypothesis test on the filtered contaminated image of a $256 \times 256$ line defected Gaussian image using various kernel radiuses. The dashed lines show the resulting 95\% empirical confidence interval when the hypothesis test was done on the uncontaminated image. Defected pixels have the distribution $\normal(3,1)$. The line defect is 5 pixels thick. The contamination is such that the null distribution is $Z_{x,y}|H_{0,x,y}\sim\normal(\mu_{0,x,y},\sigma_0^2)$ where $\mu_{0,x,y} = 0.01 (x-x_0) + 0.01 (y-y_0)$ and $\sigma_0=2$. The test was done at the 5\% FDR level. The box plot summarise the 100 repeats of the experiment.}
...
An experiment was conducted where the contaminated defected image were filtered for various kernel radiuses for a fixed alternative distribution $\normal(3,1)$. Hypothesis testing was done on the uncontaminated and filtered contaminated image and the area under the ROC curve and various errors were recorded, this was repeated 100 times. The results of the uncontaminated image were all pooled together to obtain the empirical distribution of the ROC area and errors without contamination. Results for the line and square defect are shown in Figures \ref{fig:inference_Experiment_DefectRadiusLine} and \ref{fig:inference_Experiment_DefectRadiusSquare} respectively.

The results showed that the ROC area increased with kernel radius. This highlighted that a kernel with a good radius can performance almost as though there was no contamination. It also appeared that the optimal ROC area was achieved when using the kernel radius from the rule of thumb, making these results consistent with literature.
...
\begin{figure}[htp]
  \centering
    \centerline{
      \begin{subfigure}{0.49\textwidth}
        \includegraphics[width=\textwidth]{../figures/inference/DefectDetectSep16120deg_radius1_z.eps}
        \caption{$r=\addNumber{../figures/inference/DefectDetectSep16120deg_radius1.txt}$}
      \end{subfigure}
      \begin{subfigure}{0.49\textwidth}
        \includegraphics[width=\textwidth]{../figures/inference/DefectDetectSep16120deg_radius2_z.eps}
        \caption{$r=\addNumber{../figures/inference/DefectDetectSep16120deg_radius2.txt}$}
      \end{subfigure}
    }
    \centerline{
      \begin{subfigure}{0.49\textwidth}
        \includegraphics[width=\textwidth]{../figures/inference/DefectDetectSep16120deg_radius3_z.eps}
        \caption{$r=\addNumber{../figures/inference/DefectDetectSep16120deg_radius3.txt}$}
      \end{subfigure}
      \begin{subfigure}{0.49\textwidth}
        \includegraphics[width=\textwidth]{../figures/inference/DefectDetectSep16120deg_radius4_z.eps}
        \caption{$r=\addNumber{../figures/inference/DefectDetectSep16120deg_radius4.txt}$}
      \end{subfigure}
    }
...
\begin{figure}[p]
  \centering
    \centerline{
      \begin{subfigure}[b]{0.49\textwidth}
        \includegraphics[width=\textwidth]{../figures/inference/DefectDetectSep16120deg_radius1_sig.eps}
        \caption{Image with positive pixels}
      \end{subfigure}
      \begin{subfigure}[b]{0.49\textwidth}
        \includegraphics[width=\textwidth]{../figures/inference/DefectDetectSep16120deg_radius1_logp.eps}
        \caption{$-\log p$}
      \end{subfigure}
    }
    \centerline{
      \begin{subfigure}{0.49\textwidth}
        \includegraphics[width=\textwidth]{../figures/inference/DefectDetectSep16120deg_radius1_nullMean.eps}
        \caption{Empirical null mean}
      \end{subfigure}
      \begin{subfigure}{0.49\textwidth}
        \includegraphics[width=\textwidth]{../figures/inference/DefectDetectSep16120deg_radius1_nullStd.eps}
        \caption{Empirical null std}
      \end{subfigure}
    }
  \caption{Resulting inference when using the empirical null filter with radius $r=\addNumber{../figures/inference/DefectDetectSep16120deg_radius1.txt}$. a) Highlighted in red are positive pixels at the 5\% FDR level. b) $p$-values on the log scale. c) and d) Empirical null mean and deviation.}
...
\begin{figure}[p]
  \centering
    \centerline{
      \begin{subfigure}[b]{0.49\textwidth}
        \includegraphics[width=\textwidth]{../figures/inference/DefectDetectSep16120deg_radius2_sig.eps}
        \caption{Image with positive pixels}
      \end{subfigure}
      \begin{subfigure}[b]{0.49\textwidth}
        \includegraphics[width=\textwidth]{../figures/inference/DefectDetectSep16120deg_radius2_logp.eps}
        \caption{$-\log p$}
      \end{subfigure}
    }
    \centerline{
      \begin{subfigure}{0.49\textwidth}
        \includegraphics[width=\textwidth]{../figures/inference/DefectDetectSep16120deg_radius2_nullMean.eps}
        \caption{Empirical null mean}
      \end{subfigure}
      \begin{subfigure}{0.49\textwidth}
        \includegraphics[width=\textwidth]{../figures/inference/DefectDetectSep16120deg_radius2_nullStd.eps}
        \caption{Empirical null std}
      \end{subfigure}
    }
  \caption{Resulting inference when using the empirical null filter with radius $r=\addNumber{../figures/inference/DefectDetectSep16120deg_radius2.txt}$. a) Highlighted in red are positive pixels at the 5\% FDR level. b) $p$-values on the log scale. c) and d) Empirical null mean and deviation.}
...
\begin{figure}[p]
  \centering
    \centerline{
      \begin{subfigure}[b]{0.49\textwidth}
        \includegraphics[width=\textwidth]{../figures/inference/DefectDetectSep16120deg_radius3_sig.eps}
        \caption{Image with positive pixels}
      \end{subfigure}
      \begin{subfigure}[b]{0.49\textwidth}
        \includegraphics[width=\textwidth]{../figures/inference/DefectDetectSep16120deg_radius3_logp.eps}
        \caption{$-\log p$}
      \end{subfigure}
    }
    \centerline{
      \begin{subfigure}{0.49\textwidth}
        \includegraphics[width=\textwidth]{../figures/inference/DefectDetectSep16120deg_radius3_nullMean.eps}
        \caption{Empirical null mean}
      \end{subfigure}
      \begin{subfigure}{0.49\textwidth}
        \includegraphics[width=\textwidth]{../figures/inference/DefectDetectSep16120deg_radius3_nullStd.eps}
        \caption{Empirical null std}
      \end{subfigure}
    }
  \caption{Resulting inference when using the empirical null filter with radius $r=\addNumber{../figures/inference/DefectDetectSep16120deg_radius3.txt}$. a) Highlighted in red are positive pixels at the 5\% FDR level. b) $p$-values on the log scale. c) and d) Empirical null mean and deviation.}
...
\begin{figure}[p]
  \centering
    \centerline{
      \begin{subfigure}[b]{0.49\textwidth}
        \includegraphics[width=\textwidth]{../figures/inference/DefectDetectSep16120deg_radius4_sig.eps}
        \caption{Image with positive pixels}
      \end{subfigure}
      \begin{subfigure}[b]{0.49\textwidth}
        \includegraphics[width=\textwidth]{../figures/inference/DefectDetectSep16120deg_radius4_logp.eps}
        \caption{$-\log p$}
      \end{subfigure}
    }
    \centerline{
      \begin{subfigure}{0.49\textwidth}
        \includegraphics[width=\textwidth]{../figures/inference/DefectDetectSep16120deg_radius4_nullMean.eps}
        \caption{Empirical null mean}
      \end{subfigure}
      \begin{subfigure}{0.49\textwidth}
        \includegraphics[width=\textwidth]{../figures/inference/DefectDetectSep16120deg_radius4_nullStd.eps}
        \caption{Empirical null std}
      \end{subfigure}
    }
  \caption{Resulting inference when using the empirical null filter with radius $r=\addNumber{../figures/inference/DefectDetectSep16120deg_radius4.txt}$. a) Highlighted in red are positive pixels at the 5\% FDR level. b) $p$-values on the log scale. c) and d) Empirical null mean and deviation.}
  \label{fig:inference_sep16120deg_inference_radius4}
\end{figure}

The empirical null filter was applied to the \texttt{Sep16 120deg} image as shown at the start of the chapter. Figure \ref{fig:inference_sep16120deg_zfiltered} shows the filtered $z$ images for various kernel radiuses. For $r=\addNumber{../figures/inference/DefectDetectSep16120deg_radius1.txt}$, the resulting $z$ image was smooth and treated everything as the null. When a large enough radius was used, the defects were highlighted by the higher test statistics and everything else remain unchanged. Figures \ref{fig:inference_sep16120deg_inference_radius1} to \ref{fig:inference_sep16120deg_inference_radius4} shows the resulting inference along with the empirical null parameters. The figures shows that for large enough kernel radius, the filter ironed out the false positives observed at the start of the chapter. All of the defects were successfully highlighted by the hypothesis test.

False positives were detected on the bottom right of the 3D printed sample. This was the result of the null distribution being bimodal where each face had a different distribution. Figure \ref{fig:inference_cornerSelect} illustrates this. The geometry of the faces and the kernel was such that it resulted in one of the faces to be treated as non-null and then tested positive, falsely so.

The false positives were tackled by manually segmenting the image further as shown in Figure \ref{fig:inference_segmentFurther}. It was segmented using the edges of the projection of the 3D printed object. The empirical null filter was then used on each segment, or ROI, independently, ignoring any pixels outside the ROI the filter is working on. The resulting filtered segments were stitched together to form the resulting filtered image. By using this method, the resulting inference is shown in Figure \ref{fig:inference_subroisep16120deg_inference_radius4}. It can be seen that the empirical null mean did change face to face, resulting in a clear boundary set by the edges. Most of false positives around the corners and edges have been eliminated.

\begin{figure}[htp]
  \centering
  \centerline{
    \begin{subfigure}[b]{0.49\textwidth}
      \includegraphics[width=\textwidth]{../figures/inference/cornerSelect.png}
      \caption{$z$ image with kernel of radius $r=120$}
    \end{subfigure}
    \begin{subfigure}[b]{0.49\textwidth}
      \includegraphics[width=\textwidth]{../figures/inference/cornerSelectHisto.png}
      \caption{Histogram of $z$ in the kernel}
    \end{subfigure}
  }
  \caption{A kernel of radius $r=120$ is placed over the bottom right of a unfiltered $z$ image. The histogram of the test statistics captured by the kernel shows a bimodal distribution.}
...
\begin{figure}[htp]
  ...
  \caption{The $z$ image was segmented further into 7 segments.}
...
\begin{figure}[p]
  ...
  \caption{Resulting inference when using the empirical null filter, on each segment independently, with radius $r=\addNumber{../figures/inference/DefectDetectSubRoiSep16120deg_radius4.txt}$. a) Highlighted in red are positive pixels at the 5\% FDR level. The test statistics from all segments were combined in the BH procedure. b) $p$-values on the log scale. c) and d) Empirical null mean and standard deviation.}
...
The main bottleneck is the evaluation of the density estimate, each step in the Newton-Raphson required the evaluation of $\pi r^2$ data points. Perhaps there may be faster methods for density estimation such as fitting a smoothing spline on the histogram \citep{efron2004large}, however that would require tuning the histogram bins as well as the tuning parameters for the spline. On the other hand, the method in \cite{schwartzman2008empirical} which uses the histogram count found that the estimation of the null parameters was insensitive to the histogram binning.