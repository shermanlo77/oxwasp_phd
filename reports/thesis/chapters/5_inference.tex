...
Various simulated defects were investigated. Dust defect with intensity $\pi_1$ assign all test statistics to be distributed such that
\begin{equation}
	p_{Z_{x,y}}(z)=(1-\pi_1)p_{Z_{x,y}|H_0}(z) + \pi_1 p_{Z_{x,y}|H_1}(z) \ .
\end{equation}
In other words, test statistics has the alternative distribution with probability $\pi_1$, and has the null distribution otherwise. This was chosen because the proportion of null statistics captured by a kernel should be independent of the radius of the kernel.

A line defect assign columns of pixels to have the alternative distribution, a kernel would only capture a section of defect. Similarly different shapes are used such as a square defect. With a square defect, a kernel can capture the entire defect.
