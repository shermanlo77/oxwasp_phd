...
In two-tailed hypothesis testing \citep{pearson1900on, neyman1933on, fisher1970statistical}, the $p$-value can be compared with the user defined significance level $\alpha$, also known as the size of the test in this specific example. A positive result is declared when $p<\alpha$, else it is a negative result. Correct and incorrect testing of the statistics can occur. For example testing a null statistic as positive is known as a false positive. It can be shown that $\alpha$ controls the false positive rate. $\alpha=5.0\%$ is a typical choice \citep{wasserstein2019moving} and is used throughout this thesis. Other fields use the $\sigma$ notation, for example $2\sigma$ correspond to about the $\alpha = 4.6\%$ significance level; this notation will not be used here.
...
Because the empirical null filter was implemented by modifying the existing code in the class \texttt{RankFilters}, most of the design had already been done. \texttt{RankFilters} contain implementations of filters such as the mean filter and median filter. The implementation uses multiple threads, each thread filters a row in parallel.
...
\begin{figure}[p]
  ...
  \caption{A $256 \times 256$ Gaussian image with a \emph{square} defect is shown in a). The image was contaminated in b) and then filtered with kernel radius \emph{40} in c). Defected pixels have the distribution $\normal(3,1)$. The speckle defect has density $\pi_1 = 0.1$. The line defect is 5 pixels thick. The square defect is $30\times 30$ pixels in size. The contamination is such that the null distribution is $Z_{x,y}|H_{0,x,y}\sim\normal(\mu_{0,x,y},\sigma_0^2)$ where $\mu_{0,x,y} = 0.01 (x-x_0) + 0.01 (y-y_0)$ and $\sigma_0=2$. In a) and c), highlighted in red are pixels tested as positive at the 5\% FDR level.}
  ...
  \centering
    \centerline{
      \begin{subfigure}{\subSize}
        ...
        \caption{Dust defect, $r=20$}
      \end{subfigure}
      \begin{subfigure}{\subSize}
        \includegraphics[width=\textwidth]{../figures/inference/DefectExampleLine_EmpiricalNullFilterpValueFiltered.eps}
        \caption{Line defect, $r=20$}
      \end{subfigure}
    }
    \centerline{
      \begin{subfigure}{\subSize}
        \includegraphics[width=\textwidth]{../figures/inference/DefectExampleSquare20_EmpiricalNullFilterpValueFiltered}
        \caption{Square defect, $r=20$}
      \end{subfigure}
      \begin{subfigure}{\subSize}
        \includegraphics[width=\textwidth]{../figures/inference/DefectExampleSquare40_EmpiricalNullFilterpValueFiltered}
        \caption{Square defect, $r=40$}
      \end{subfigure}
    }
  \caption{A $256 \times 256$ contamined defected Gaussian image was filtered using the empirical null filter with kernel radius $r$. Shown are the values of the image converted to $p$-values. Defected pixels have the distribution $\normal(3,1)$. The square defect is $30\times 30$ pixels in size. The contamination is such that the null distribution is $Z_{x,y}|H_{0,x,y}\sim\normal(\mu_{0,x,y},\sigma_0^2)$ where $\mu_{0,x,y} = 0.01 (x-x_0) + 0.01 (y-y_0)$ and $\sigma_0=2$. The critical region is for testing at the 5\% FDR level.}
...
The empirical null mean in Figure \ref{fig:inference_defectSquareExample} demonstrates the multi-thread nature of the implementation of the empirical null filter. It can be seen that there are horizontal streaks where the defect is. This is the result of each thread filtering a row each and jumping from one mode, the null, to the other, the non-null, at different times. The horizontal streaks can be removed by using more initial values so that the Newton-Raphson method can pin point which mode is greater.
...
\begin{figure}[p]
  \centering
    \centerline{
      \begin{subfigure}[b]{\subSize}
        \includegraphics[width=1.05\textwidth, height=5.6cm]{../figures/inference/DefectDetectAbsFilterDeg120_radius1_sig.eps}
...
\begin{figure}[p]
  \centering
    \centerline{
      \begin{subfigure}[b]{\subSize}
        \includegraphics[width=1.05\textwidth, height=5.6cm]{../figures/inference/DefectDetectAbsFilterDeg120_radius4_sig.eps}
        \caption{Image with positive pixels}
      \end{subfigure}
      \begin{subfigure}[b]{\subSize}
...
\begin{figure}[p]
  \centering
    \centerline{
      \begin{subfigure}[b]{\subSize}
        \includegraphics[width=1.05\textwidth, height=5.6cm]{../figures/inference/DefectDetectSubRoiAbsFilterDeg120_radius4_sig.eps}
        \caption{Image with positive pixels}
      \end{subfigure}
      \begin{subfigure}[b]{\subSize}
...
\begin{figure}[p]
  \centering
    \centerline{
      \begin{subfigure}[b]{\subSize}
        \includegraphics[width=1.05\textwidth, height=5.6cm]{../figures/inference/DefectDetectSubRoiAbsFilterDeg30_radius1_sig.eps}
        \caption{Image with positive pixels}
      \end{subfigure}
      \begin{subfigure}[b]{\subSize}
...
\begin{figure}[p]
  \centering
    \centerline{
      \begin{subfigure}[b]{\subSize}
        \includegraphics[width=1.03\textwidth, height=5.6cm]{../figures/inference/DefectDetectSubRoiTiFilterDeg30_radius1_sig.eps}
        \caption{Image with positive pixels}
      \end{subfigure}
      \begin{subfigure}[b]{\subSize}
...
\begin{figure}[p]
  \centering
    \centerline{
      \begin{subfigure}[b]{\subSize}
        \includegraphics[width=1.03\textwidth, height=5.6cm]{../figures/inference/DefectDetectSubRoiTiFilterDeg120_radius1_sig.eps}
        \caption{Image with positive pixels}
      \end{subfigure}
      \begin{subfigure}[b]{\subSize}
...
While the empirical null filter performed well for the ABS test sample, it failed in the titanium test sample due to dominating effects on the edges. This could be due to poor face segmentation or poor alignment of the \emph{aRTist} projection with the obtained projection. This demonstrates that the empirical null filter can cater for model misspecification but there becomes a point where model misspecified null statistics cannot be distinguished from non-null statistics. Another possible cause for dominating edge effects are metal artifacts such as streaks which are problems observed in XCT of metal parts \citep{deman1998metal, nawaz2014metal}. Lastly, because the titanium test sample was created using a power-based method, the voids may be filled with power thus appear with less contrast in the projections making it hard to detect.
...
Estimation of the null parameters is essentially robust statistics, estimating the parameters of the null distribution without being affected by non-null statistics. Potential faster methods than the empirical null could exist in literature for robust statistics such as \cite{hampel1986robust, rousseeuw1987robust, maronna2006robust, huber2009robust, jewson2018principles}. The use of density estimation for robust estimation is also featured when using loss functions derived from the Hellinger-divergence \citep{beran1977minimum, jewson2018principles} or the beta-divergence \citep{basu1998robust, jewson2018principles} which bears similarities to the empirical null. However, numerical methods are still required to find the mode. Exact solutions for the mode using Bayesian methods is impossible \citep{heinrich2013the} which suggest avoiding the use of numerical methods can be difficult.
