...
\begin{figure}
  ...
  \begin{subfigure}[b]{\subSize}
      ...
      \caption{$\sigma_0$}
  \end{subfigure}
  \caption{Median squared error of the estimates of the empirical null parameters over 100 repeats of $n$ simulated standard Normal data. Lines represent the rule of thumb for various values of $b$.}
...
\begin{figure}
  ...
  \caption{An identity link gamma GLM was fitted onto the relationship between the optimal bandwidth and $n^{-1/5}$ from the experiment. The dashed lines shows the 68\% prediction interval.}
...
The empirical null filter was tested to see if it can assist in detecting simulated defects from an image with/without contamination. A defect assign pixels to have a value not distributed under the null distribution, but instead, under an alternative, or non-null, distribution. For example, suppose $Z_{x,y}|H_{0,x,y}\sim\normal(0,1)$, then a defect assign certain pixels to have a value distributed as $Z_{x,y}|H_{1,x,y}\sim\normal(\mu_{1},1)$ where $\mu_1\neq0$.
...
The ROC curves consider all thresholds or specificities used in the hypotheses testing. In the previous experiment, it was found that the variance of the test statistics were different before and after filtering. As a result, for a given threshold, such as the 5\% FDR level, the specificity may change ever so slightly after filtering.
...
\begin{figure}
  ...
  \caption{AUC and various errors obtained when conducting hypotheses testing on an empirical null filtered contaminated salt defected Gaussian image of size $\SI{256}{\pixel} \times \SI{256}{\pixel}$. In a), the upper/lower dot dashed lines shows the resulting mean AUC when testing on a defected image before/after contamination respectively. In b), c) and d), the dashed lines show the 95\% empirical confidence interval of the resulting error when testing the defected image before contamination. The filter used a kernel radius of \SI{20}{\pixel}. Defected pixels have the distribution $\normal(\mu_1,1)$ where $\mu_1$ was varied in this experiment. The salt defect has density $\pi_1 = 0.1$. The contamination was such that the null distribution is $Z_{x,y}|H_{0,x,y}\sim\normal(\mu_{0,x,y},\sigma_0^2)$ where $\mu_{0,x,y} = 0.01 (x-x_0) + 0.01 (y-y_0)$ and $\sigma_0=2$. The test was done at the 5\% FDR level. The boxplots summarise the 100 repeated simulations of the image.}
