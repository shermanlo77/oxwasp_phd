...
The resulting 100 bootstrapped scaled deviance are shown in Figure \ref{fig:meanvar_deviance_mar16}. The scaled deviance does not vary much from model to model and suggests that the goodness of fit are all very similar.
...
In this particular example, $m\approx 2\times 10^6$ and $n=100$. Then one would expect the scaled deviance to have values
\begin{align*}
    \expectation\left[D^*\right]&=\frac{1}{\alpha}\expectation\left[D\right]
    \\
    &=\frac{2}{n-1}\times (m-2)
    \\
    &\approx \frac{2}{100}\times 2\times 10^6
    \\
    &\approx 4 \times 10^4
\end{align*}
which is similar in magnitude obtained from the bootstrap samples.
